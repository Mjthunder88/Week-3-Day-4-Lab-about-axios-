...
function changeHelloColor () {
    sayHelloButton.classList.add('change-theme')
}
...
// CODE HERE
function revert () {
    sayHelloButton.classList.remove('change-theme')
}
...
const ohMy = () => {
    ...
    .then((response) => {
       for (let i = 0; i < response.data.length; i++) {
        ...
        bodyEle.appendChild(newElement)
       }
    })
    .catch()
}
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/Marcus')
    .then((response) => {
...
let newButton = document.querySelector('#repeat-button')
newButton.addEventListener('click',repeatMyParam)
...
function makeRequest () {
    axios.get('http://localhost:3000/query-test?name=Marcus')
    .then ((response) => {
       console.log(response.data)
    })
    .catch(() => {
...
let buttonQ = document.getElementById('query-button')
buttonQ.addEventListener('click', makeRequest)
...
/*
    ...
    // said you sent in no query 
    2: Send more than 1 query on the URL -- what happened? 
    // just added the secound query onto the first one. So it said Marcus?age=20
*/
